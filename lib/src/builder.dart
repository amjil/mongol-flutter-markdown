...
class MarkdownBuilder implements md.NodeVisitor {
  ...
  void visitElementAfter(md.Element element) {
    ...
    if (_isBlockTag(tag)) {
      ...
      if (builders.containsKey(tag)) {
        ...
        MongolTextAlign? malign;
        if (alignAttribute == null) {
          //align = tag == 'th' ? styleSheet.tableHeadAlign : MongolTextAlign.top;
          malign = MongolTextAlign.top;
        } else {
          switch (alignAttribute) {
            case 'left':
              malign = MongolTextAlign.top;
            case 'center':
              malign = MongolTextAlign.center;
            case 'right':
              malign = MongolTextAlign.bottom;
          }
        }
        final Widget child = _buildTableCell(
          _mergeInlineChildren(current.children, malign),
          textAlign: align,
        );
  ...
  Iterable<TextSpan> _getInlineSpans(TextSpan span) {
    ...
    final Iterable<TextSpan> spans = span.children!.map((InlineSpan childSpan) {
  ...
  List<Widget> _mergeInlineChildren(
    ...
    for (final Widget child in children) {
      ...
      if (last is MongolText) {
        final TextSpan span = last.textSpan!;
        spans.addAll(_getInlineSpans(span));
      } else if (last is MongolRichText) {
        final TextSpan span = last.text;
        spans.addAll(_getInlineSpans(span));
      } else {
      ...
      if (child is MongolText) {
        final TextSpan span = child.textSpan!;
        spans.addAll(_getInlineSpans(span));
        /*} else if (child is SelectableText) {
        final TextSpan span = child.textSpan!;
        spans.addAll(_getInlineSpans(span));*/
      } else if (child is MongolRichText) {
        final TextSpan span = child.text;
        spans.addAll(_getInlineSpans(span));
      } else {
      ...
      if (spans.isNotEmpty) {
        ...
        /*if (selectable) {
          ...
          mergedTexts.add(MongolText.rich(
            child,
            textScaler: styleSheet.textScaler,
            textAlign: textAlign ?? TextAlign.start,
          ));
  ...
  List<TextSpan> _mergeSimilarTextSpans(List<TextSpan> textSpans) {
    if (textSpans.length < 2) {
      return textSpans;
    }

    final List<TextSpan> mergedSpans = <TextSpan>[];

    for (int index = 1; index < textSpans.length; index++) {
      final TextSpan previous =
      ...
      if (!previousIsTextSpan || !nextIsTextSpan) {
        mergedSpans.addAll(<TextSpan>[previous, nextChild]);
        continue;
      }
  ...
  Widget _buildRichText(TextSpan? text,
      {MongolTextAlign? textAlign, String? key}) {
